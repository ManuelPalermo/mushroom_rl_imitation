...
class GAIL(PPO):
    ...
    def _fit_discriminator(self, plcy_obs, plcy_act):
        for _ in range(self._n_epochs_discriminator):
            ...
            for plcy_obs_i, plcy_act_i, demo_obs_i, demo_act_i in \
                ...
                d_demo_target = torch.FloatTensor(plcy_obs_i.shape[0], 1).uniform_(0.90, 0.99).to(self._device)
                d_plcy_target = torch.FloatTensor(demo_obs_i.shape[0], 1).uniform_(0.01, 0.10).to(self._device)
    ...
    def _make_discrim_reward(self, state, action):
        ...
        with torch.no_grad():
            d_plcy = self._discr_approx(state_action).detach().cpu().numpy()
            return np.squeeze(-np.log(1 - d_plcy))
