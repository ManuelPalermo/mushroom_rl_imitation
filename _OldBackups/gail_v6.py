    ...
    def _fit_discriminator(self, plcy_obs, plcy_act):
        for _ in range(self._n_epochs_discriminator):
            ...
            for plcy_obs_i, plcy_act_i, demo_obs_i, demo_act_i in \
                ...
                d_demo_target = torch.zeros((demo_obs_i.shape[0], 1), device=self._device)   # no label smoothing
                d_plcy_target = torch.ones((plcy_obs_i.shape[0], 1), device=self._device)  # no label smoothing
