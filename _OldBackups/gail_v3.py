...
class GAIL(PPO):
    ...
    def fit(self, dataset):
        ...
        if self._use_discriminator:
            # use reward from the discriminator
    ...
    def _fit_discriminator(self, plcy_obs, plcy_act):
        for _ in range(self._n_epochs_discriminator):
            ...
            '''
            # create targets with smooth labels: (demos(~1) or policy(~0))
            d_demo_target = torch.FloatTensor(demo_obs.shape[0], 1).uniform_(0.90, 0.95).to(self._device)
            d_plcy_target = torch.FloatTensor(plcy_obs.shape[0], 1).uniform_(0.05, 0.10).to(self._device)

            #d_plcy_target = torch.ones((plcy_obs.shape[0], 1), device=self._device)
            #d_demo_target = torch.zeros((demo_obs.shape[0], 1), device=self._device)

            # discriminated samples(classified into expert demos(~1) or policy(~0))
            d_demo = self._discr_approx(torch.cat([demo_obs, demo_act], dim=1))
            d_plcy = self._discr_approx(torch.cat([plcy_obs, plcy_act], dim=1))

            # calculate and propagate discriminator loss
            discr_loss = self._discr_loss(d_demo, d_demo_target) + \
                         self._discr_loss(d_plcy, d_plcy_target)
            ...
            for plcy_obs_i, plcy_act_i, demo_obs_i, demo_act_i in \
                ...
                d_demo_target = torch.FloatTensor(plcy_obs_i.shape[0], 1).uniform_(0.05, 0.10).to(self._device)
                d_plcy_target = torch.FloatTensor(demo_obs_i.shape[0], 1).uniform_(0.80, 0.95).to(self._device)
    ...
    def _make_discrim_reward(self, state, action):
        ...
        with torch.no_grad():
            return np.squeeze(-np.log(1-self._discr_approx(state_action).detach().cpu().numpy()))