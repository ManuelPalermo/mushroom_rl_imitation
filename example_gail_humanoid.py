...
def _load_demos_selected_discriminator(mdp_info):
    # load expert training data -> (select only joints from trajectories to compare(qpos/qvel))
    expert_files = np.load("examples/expert_data/humanoid_gait_trajectory.npz")
...
def init_policy_with_bc(agent):
    # load expert training data -> (select only joints from trajectories to compare(qpos/qvel))
    expert_files = np.load("examples/expert_data/expert_dataset_humanoid_muscles.npz")
...
def experiment(algorithm, init_bc=False):
    ...
    plt.figure()
    plt.plot(epoch_js)
    plt.savefig("train_plot_{}".format(algorithm))
    input()

    # evaluate trained policy
    dataset = core.evaluate(n_episodes=25, quiet=True)
